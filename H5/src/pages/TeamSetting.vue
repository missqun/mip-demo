<template>
  <div class='g-inherit m-article p-setting'>
    ...
    <group>
      ...
      <x-input title="群昵称"  v-if='config.inputType==="text"' :placeholder="placeHolder" v-model='inputModel' ref='input' :disabled='!config.enable' :max='8'></x-input>
    ...
    <group class="update_btn">
       <x-button type="primary" v-if='config.inputType!=="select" && config.enable'  @click.native='() => update()'>保存</x-button>
    </group>
    </group>
  </div>
</template>
