<template>
  <div class='g-inherit m-article p-teammanager'>
    ...
    <x-header class="m-tab" :left-options="{backText: ' '}" :right-options="{showMore: true}" @on-click-more="showMenu=true">
      <h1 class="m-tab-top">上上计划群聊</h1>
      <a slot="left"></a>
    </x-header>
    ...
    <div class='m-body'>
      ...
      <template v-if="teamInfo && teamInfo.type==='advanced'">
        <cell is-link @click.native='onTeamAvatarClick' class="teaminfo_box">
          <div class='m-teaminfo' slot='icon'>
            <img class='avatar u-circle' :src='teamAvatar'>
            <div class='u-info'>
              <p>{{teamInfo.name}}</p>
              <p class="dark_text">{{teamInfo.teamId}}</p>
              <!-- <p>{{于`${formateDate(teamInfo.createTime)}创建`}}</p> -->
              <p class="dark_text">于2018-12-17创建</p>
            </div>
          </div>
          <form>
        ...
        <group class="my_group_card auto_cell">
           <cell title="我的群名片" is-link link="/teammembercard/1526847681-zxcvbnm321">
                <p class="dark_text">点击设置</p>
           </cell>
        </group>
        <group class='m-group auto_cell t_member'>
        ...
        <group class='m-group auto_cell'>
          <cell title="群名称" :value="teamName" @click.native="()=>onEditItemClick(hasEditPermission?'修改群名称':'群名称', 'text', 'name')" ></cell>
          <!-- <cell title="群昵称" :value="nickName" @click.native="()=>onEditItemClick('修改群昵称', 'text', 'nickInTeam', true)" is-link></cell> -->
          <cell title="群介绍" :value="teamInfo.intro || '暂无'" @click.native="()=>onEditItemClick(hasEditPermission?'修改群介绍':'群介绍', 'textarea', 'intro')" is-link></cell>
          <cell title="群公告" :value="teamInfo.intro || '暂无'" @click.native="()=>onEditItemClick(hasEditPermission?'修改群介绍':'群介绍', 'textarea', 'intro')" is-link></cell>
          <x-switch title="消息提醒" v-model="notice1"></x-switch>
        </group>
        <group class='m-group auto_cell' v-if='hasManagePermission'>
        ...
        <group class="group_manager">
           <cell title="群管理" is-link link="/teamownmanage"></cell>
        </group>
      </template>
    </div>
